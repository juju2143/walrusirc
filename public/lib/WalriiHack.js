...
function init(fps)
{
	for (var i = 0; i < 10; i++)
	{
		...
		walriis.push(new Walrii(Math.random()*(window.innerWidth - 24), Math.random()* (window.innerHeight - 26), s * Math.cos(r), s * Math.sin(r)));
	}
...
function Walrii(x, y, vx, vy)
{
	...
	this.img.src = "http://codewalr.us/other/hypergarfield.gif";
	this.img.width = 24;
	this.img.height = 26;
	...
	this.update = function()
	{
		...
		this.vy *= ((this.y < 0  && this.vy < 0) || (this.y + 26 > window.innerHeight && this.vy > 0)) ? -1 : 1;
		if (collide)
			this.collide();
		this.img.style.left =  Math.round(this.x)+"px";
		this.img.style.top =   Math.round(this.y)+"px";
